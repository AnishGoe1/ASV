...
# ── Z-N Method 1 PID parameters ───────────────────────────────────────────────
_R  = K_MODEL / T_MODEL
_L  = 0.1 * T_MODEL
KP_DEFAULT = 1.2  / (_R * _L)
KI_DEFAULT = KP_DEFAULT / (2.0 * _L)
KD_DEFAULT = KP_DEFAULT * (0.5 * _L)
...
# ── Heading deadband ──────────────────────────────────────────────────────────
# If |error| < this threshold the PID output is zeroed — thrusters stay silent.
# Prevents chattering from IMU noise when close to the setpoint.
# Tune to be slightly larger than your IMU noise floor (degrees).
...
class ASVController(Node):
    ...
    def control_loop(self):
        ...
        # ── Heading deadband ──────────────────────────────────────────────────
        # Zero out small errors to prevent chattering from IMU noise.
        # Integral is also frozen inside the deadband to avoid windup.
        in_deadband = abs(error) < HEADING_DEADBAND
        if in_deadband:
            error = 0.0

        P = self.Kp * error

        # Freeze integral during open-loop turns or inside deadband
        # — prevents windup while PID output is suppressed
        if not self.turning and not in_deadband:
            self.integral = clamp(
        ...
        if self.turning:
            ...
            if in_deadband:
                mode_str += "*"   # asterisk = inside deadband
